      ...
      value: function getMouseData(scope) {
        ...
        if (airMode) {
          scene.addEventListener('mousemove', function () {
            var rect = getRect(el);
            var maxX = window.innerWidth + rect.width / 2;
            var maxY = window.innerHeight + rect.height / 2;
            var distanceX = mouseData.mouseX - rect.centerX;
            var distanceY = mouseData.mouseY - rect.centerY;
