(function (global, factory) {
  ...
  function getRectTop(target) {
    return target.getBoundingClientRect().top;
  }
  function getRectLeft(target) {
  ...
  var Cotton = /*#__PURE__*/function () {
    ...
    _createClass(Cotton, [{
      ...
      function move() {
        ...
        function animateMouse() {
          ...
          if (params.airMode) {
            ...
            if (_typeof(airMode) !== 'object' || Array.isArray(airMode)) airMode = (_readOnlyError("airMode"), {
              resistance: 15,
              reverse: false
            });
            var distanceX = airMode.reverse ? -(mouseData.x - (getRectLeft(el) + getRectWidth(el) / 2)) : mouseData.x - (getRectLeft(el) + getRectWidth(el) / 2);
            var distanceY = airMode.reverse ? -(mouseData.y - (getRectTop(el) + getRectHeight(el) / 2)) : mouseData.y - (getRectTop(el) + getRectHeight(el) / 2);
            el.style.transform = "translate(".concat(distanceX / airMode.resistance, "px, ").concat(distanceY / airMode.resistance, "px)");
          } else el.style.transform = "translate(calc(-50% + ".concat(mouseData.x, "px), calc(-50% + ").concat(mouseData.y, "px))");
        }
      ...
      value: function init(scope) {
        var scene = document.querySelector(scope.params.scene);
