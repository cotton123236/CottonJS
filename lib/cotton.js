(function (global, factory) {
  ...
  function cottonAnimation(scope) {
    ...
    el.style.transform = "translate(calc(-50% + ".concat(mouseData.x, "px), calc(-50% + ").concat(mouseData.y, "px))");
  }
      ...
      value: function setData(scope, type) {
        ...
        if (airMode) {
          ...
          function getMouseDistance(e) {
            ...
            if (distanceX > maxX) mouseData.distanceX = maxX;else if (distanceX < -maxX) mouseData.distanceX = -maxX;else mouseData.distanceX = distanceX;
            if (distanceY > maxY) mouseData.distanceY = maxY;else if (distanceY < -maxY) mouseData.distanceY = -maxY;else mouseData.distanceY = distanceY;
          }
