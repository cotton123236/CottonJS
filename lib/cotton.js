    ...
    _createClass(Cotton, [{
      ...
      value: function move() {
        ...
        function animateMouse() {
          ...
          if (params.bloomMode) {
            if (_typeof(params.bloomMode) !== 'object' || Array.isArray(params.bloomMode)) params.bloomMode = {
              resistance: 15,
              reverse: false
            };
            var distanceX = params.bloomMode.reverse ? -(mouseData.x - (getRectLeft(el) + getRectWidth(el) / 2)) : mouseData.x - (getRectLeft(el) + getRectWidth(el) / 2);
            var distanceY = params.bloomMode.reverse ? -(mouseData.y - (getRectTop(el) + getRectHeight(el) / 2)) : mouseData.y - (getRectTop(el) + getRectHeight(el) / 2);
            el.style.transform = "translate(".concat(distanceX / params.bloomMode.resistance, "px, ").concat(distanceY / params.bloomMode.resistance, "px)");
